...
// Etiqueta contextual del "descanso entre repeticiones" según el tipo de ejercicio
const REPS_REST_LABEL: Record<BlockType, string> = {
  intervals: 'Descanso entre Repeticiones',
  reps: 'Descanso entre Repeticiones',
  problems: 'Descanso entre Intentos',
  attempts: 'Descanso entre Intentos',
  free: 'Descanso entre Repeticiones',
};
...
export function BlockFormInline({
  ...
  const [activeType, setActiveType] = useState<BlockType>(initialBlock?.type || 'intervals');
  ...
  useEffect(() => {
    if (initialBlock) {
      setActiveType(initialBlock.type);
      reset(blockToFormValues(initialBlock));
    } else {
      setActiveType('intervals');
      reset(emptyValues);
    }
  }, [initialBlock, reset]);

  const watchedValues = watch();
  const repsRestLabel = REPS_REST_LABEL[activeType];
  ...
  const onSubmit = (data: PrescriptionFormValues) => {
    const block: WorkoutBlock = {
      ...
      restBetweenRepsSeconds: activeType === 'free' ? undefined : toSeconds(data.restRepsMinutes, data.restRepsSeconds),
  ...
  const previewBlock: WorkoutBlock = {
    ...
    restBetweenRepsSeconds: toSeconds(watchedValues.restRepsMinutes, watchedValues.restRepsSeconds),
      ...
      <div className="p-4 bg-chalk-50 dark:bg-graphite-850 rounded-2xl border border-chalk-200 dark:border-graphite-750 space-y-3.5">
        {activeType === 'intervals' && (
          <>
            ...
            <div className="pt-2">
              <Input type="number" min={1} label="Repeticiones por Serie (Opcional, ej. 7/3)" placeholder="Ej: 6" {...register('repetitions')} />
            </div>
          </>
        )}
        ...
        {activeType === 'attempts' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input type="number" min={1} label="Series" placeholder="Ej: 3" {...register('sets')} />
        ...
        {/* Descanso entre series y entre repeticiones: disponible para todos los tipos salvo Libre */}
        {activeType !== 'free' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-chalk-200 dark:border-graphite-800">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-graphite-700 dark:text-graphite-300 mb-1.5">

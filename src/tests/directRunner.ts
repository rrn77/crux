...
async function main() {
  ...
  // 1. Tests de Duración
  await test('ULAC duración estimada', () => {
  ...
  await test('BLOQUES CORTOS duración estimada', () => {
  ...
  await test('Suspensiones 20mm duración estimada', () => {
  ...
  await test('Core 3x12 duración estimada', () => {
  ...
  await test('Inicialización de temporizador', () => {
  ...
  await test('Transición trabajo -> descanso al expirar', () => {
    const block: WorkoutBlock = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 3, workDurationSeconds: 180, restDurationSeconds: 60 };
  ...
  await test('Transición descanso -> serie 2 trabajo', () => {
    const block: WorkoutBlock = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 3, workDurationSeconds: 180, restDurationSeconds: 60 };
  ...
  await test('Fin de bloque tras última serie', () => {
  ...
  await test('Pausar y reanudar con timestamps', () => {
    const block: WorkoutBlock = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 2, workDurationSeconds: 180, restDurationSeconds: 60 };
  ...
  await test('Salto de fase (1 tap skip)', () => {
    const block: WorkoutBlock = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 2, workDurationSeconds: 180, restDurationSeconds: 60 };
  ...
  await test('Recuperación precisa tras bloqueo de pantalla', () => {
  ...
  await test('Recuperación con auto-transición tras expiración en background', () => {
    const block: WorkoutBlock = { id: '1', position: 0, title: 'Suspensiones 20mm', type: 'intervals', sets: 6, workDurationSeconds: 7, restDurationSeconds: 180 };
  ...
  await test('calculateTestDelta: higher_is_better (más es mejor: kg, reps)', () => {
  ...
  await test('calculateTestDelta: lower_is_better (menos es mejor: mm, tiempo)', () => {
  ...
  await test('calculateTestFatigue: peakValue y pérdida según targetMetric', () => {
  ...
  await test('getLocalDateIsoString: formato YYYY-MM-DD correcto', () => {
    const d = new Date(2026, 8, 15); // Septiembre 15, 2026
    const iso = getLocalDateIsoString(d);
    assert.strictEqual(iso, '2026-09-15');
  });

  await test('workoutStore: scheduleSession y getSessionsByDate', async () => {
    const store = useWorkoutStore.getState();
    store.clearWorkoutStore();

    const sessionTomorrow = await store.scheduleSession({
  ...
  await test('workoutStore: getTodaySession prioridad programada/en progreso', async () => {
    const store = useWorkoutStore.getState();
    store.clearWorkoutStore();
  ...
  await test('workoutStore: deleteSession elimina correctamente la sesión por ID', async () => {
    ...
    const session = await useWorkoutStore.getState().scheduleSession({
      title: 'Fuerza Bloque',
    ...
    await useWorkoutStore.getState().deleteSession(session.id);

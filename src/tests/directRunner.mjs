// Runner directo con Node test / assert nativo para verificación rápida y determinista
...
import {
  ...
  formatDurationHuman,
  formatBlockSummary,
} from '../lib/timer/durationHelper.ts';
import {
...
test('ULAC duración estimada', () => {
  const block = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 4, workDurationSeconds: 180, restDurationSeconds: 60 };
...
test('BLOQUES CORTOS duración estimada', () => {
  const block = { id: '2', position: 0, title: 'BLOQUES CORTOS', type: 'problems', problems: 4, movements: 5, attempts: 4, restDurationSeconds: 90 };
...
test('Suspensiones 20mm duración estimada', () => {
  const block = { id: '3', position: 0, title: 'Suspensiones 20 mm', type: 'intervals', sets: 6, workDurationSeconds: 7, restDurationSeconds: 180 };
...
test('Core 3x12 duración estimada', () => {
  const block = { id: '4', position: 0, title: 'Core', type: 'reps', sets: 3, repetitions: 12, restDurationSeconds: 60 };
...
test('Inicialización de temporizador', () => {
  const block = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 3, workDurationSeconds: 180, restDurationSeconds: 60 };
...
test('Transición descanso -> serie 2 trabajo', () => {
  ...
  const trans = transitionOnTimerExpired(resting, block, startMs);
...
test('Fin de bloque tras última serie', () => {
  const block = { id: '1', position: 0, title: 'ULAC', type: 'intervals', sets: 2, workDurationSeconds: 180, restDurationSeconds: 60 };
  ...
  const trans = transitionOnTimerExpired(lastRest, block, startMs);
...
test('Recuperación precisa tras bloqueo de pantalla', () => {
  const block = { id: '1', position: 0, title: 'Suspensiones 20mm', type: 'intervals', sets: 6, workDurationSeconds: 7, restDurationSeconds: 180 };
